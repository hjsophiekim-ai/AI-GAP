...
def _safe_create_broker(cfg, mode, confirm_text="", runtime_real_mode=False):
    """구/신 버전 broker_factory 양쪽 호환 — runtime_real_mode 없는 구버전도 동작."""
    try:
            ...
            if st.button(
                ...
                try:
                    ...
                    with st.spinner("브로커 초기화 중..."):
                        broker = _safe_create_broker(cfg=cfg, mode=selected_mode, confirm_text=confirm_text, runtime_real_mode=_runtime_real_mode)
