...
def _action_summary(
    ...
    if flag == STRONG_BUY:
        ...
        return f"종합 판단: {label} ({score:.0f}점) — 단기 고점 확률 {top_p:.0f}%로 보유 물량 분할매도를 고려하세요."
    elif flag == SELL:
        return f"종합 판단: {label} ({score:.0f}점) — 추가 하락 리스크가 있어 매도 후 관망을 권장합니다."
    elif flag == STRONG_SELL:
        return f"종합 판단: {label} ({score:.0f}점) — 단기 고점 확률 {top_p:.0f}%로 즉시 손절/매도를 검토하세요."
